...
def init_images(app_data):
    ...
    IN_U1 = (np.array(u1)).astype(np.float64).ravel()
    IN_U2 = (np.array(u2)).astype(np.float64).ravel()
    IN_V1 = (np.array(v1)).astype(np.float64).ravel()
    ...
    img_data['IN_U2'] = IN_U2
    img_data['IN_V1'] = IN_V1
    img_data['IN_V2'] = IN_V2
    img_data['IN_Y'] = IN_Y
    img_data['IN_Z'] = IN_Z
