...
def fir_high_pass(pipe_data):
    ...
    scaled_coeffs.defn = [ Case(cond1, (1 - F) * win(x)), \
                           Case(cond2, \
                            -Sin((x - middle) * Pi() * F) / ((x - middle) * Pi()) \
                            * win(x)) ]
    ...
    coeffs_sum.defn = [ Reduce(coeffs_sum(z), scaled_coeffs(y) * Cos((y - middle) * Pi()), Op.Sum) ]

    out_coeffs = Wave(Double, "out_coeffs", N, x)
    out_coeffs.defn = [ scaled_coeffs(x) / coeffs_sum(0) ]

    out_coeffs2 = Wave.firwin(N, F, "out_coeffs2", pass_zero=False)
