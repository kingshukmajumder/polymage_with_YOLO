...
from polymage_unwanted_spectral import unwanted_spectral
...
def build_unwanted_spectral(app_data):
    pipe_data = app_data['pipe_data']

    out_unwanted_spectral = unwanted_spectral(pipe_data)
    
    N = pipe_data['N']
    F = pipe_data['F']

    live_outs = [out_unwanted_spectral]
    pipe_name = app_data['app']

    length = app_data['length']
    freq = app_data['freq']

    p_estimates = [(N, length), (F, freq)]
    p_constraints = []
    #~ t_size = [16, 16]
    #~ g_size = 1
    opts = []
