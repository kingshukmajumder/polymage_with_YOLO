...
from polymage_fir_band_stop import fir_band_stop
...
def build_fir_band_stop(app_data):
    pipe_data = app_data['pipe_data']

    out_fir_band_stop, ofbs2 = fir_band_stop(pipe_data)
    ...
    live_outs = [out_fir_band_stop, ofbs2]
    ...
    p_estimates = [(N, length), (FL, low_cutoff_freq), (FH, high_cutoff_freq), (typ, win_type)]
    p_constraints = [ Condition(typ, '<', 9), Condition(FL, '<=', FH) ]
    #~ t_size = [16, 16]
    #~ g_size = 1
    #~ opts = []
