...
def fir_band_stop(pipe_data):
    ...
    scaled_coeffs.defn = [ Case(cond1, (1 - (FH - FL)) * win(x)), \
                           Case(cond2, \
                            (Sin((x - middle) * Pi() * FL) - Sin((x - middle) * Pi() * FH)) / ((x - middle) * Pi()) \
                            * win(x)) ]
    ...
    coeffs_sum.defn = [ Reduce(coeffs_sum(z), scaled_coeffs(y), Op.Sum) ]

    out_coeffs = Wave(Double, "out_coeffs", N, x)
    out_coeffs.defn = [ scaled_coeffs(x) / coeffs_sum(0) ]

    out_coeffs2 = Wave.firwin(N, (FL, FH), "out_coeffs2")
