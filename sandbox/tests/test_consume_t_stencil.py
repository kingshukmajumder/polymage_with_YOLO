...
def test_consume_t_stencil_1d():
    ...
    g = Function(([x], [row]), Float, "f")
    g.defn = [ 2.0*tstencil(x) + 3 ]
    ...
    # build the pipeline
    pipeline = buildPipeline([g],
                             param_estimates = p_est,
                             pipe_name = "consume_tstencil_1d",
                             options = opts)

    filename = "test_consume_t_stencil_1d"

    dump_cpp_and_graph(pipeline, filename)

    return

def test_consume_t_stencil_2d():
    ...
    g = Function(([x, y], [row, col]), Float, "f")
    g.defn = [ 2.0*tstencil(x, y) + 3 ]
    ...
    pipeline = buildPipeline([g],
                             param_estimates = p_est,
                             pipe_name = "consume_tstencil_2d")

    filename = "test_consume_t_stencil_2d"

    dump_cpp_and_graph(pipeline, filename)

    return

def test_consume_t_stencil_3d():
    ...
    g = Function(([x, y, z], [plane, row, col]), Float, "f")
    g.defn = [ 2.0*tstencil(x, y, z) + 3 ]
    ...
    pipeline = buildPipeline([g],
                             param_estimates = p_est,
                             pipe_name = "consume_tstencil_3d")

    filename = "test_consume_t_stencil_3d"
